import { Menu, X } from "lucide-react";
import { useState } from "react";

export default function Navbar() {
  const [open, setOpen] = useState(false);

  return (
    <header className="sticky top-0 z-40 backdrop-blur-md bg-white/70 border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          <a href="#" className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 via-sky-500 to-cyan-400 shadow-lg" />
            <span className="text-slate-900 font-semibold tracking-tight">Zagosa CRM</span>
          </a>

          <nav className="hidden md:flex items-center gap-8 text-sm">
            <a href="#features" className="text-slate-600 hover:text-slate-900 transition-colors">Features</a>
            <a href="#testimonials" className="text-slate-600 hover:text-slate-900 transition-colors">Customers</a>
            <a href="#pricing" className="text-slate-600 hover:text-slate-900 transition-colors">Pricing</a>
            <a href="#faq" className="text-slate-600 hover:text-slate-900 transition-colors">FAQ</a>
          </nav>

          <div className="hidden md:flex items-center gap-3">
            <a href="https://app.zagosacrm.com/login" className="text-slate-700 hover:text-slate-900 text-sm">Sign in</a>
            <a href="https://app.zagosacrm.com/register" className="inline-flex items-center rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white shadow hover:bg-slate-800 transition">Get started</a>
          </div>

          <button className="md:hidden inline-flex items-center justify-center w-9 h-9 rounded-md border border-slate-300 text-slate-700" onClick={() => setOpen(!open)} aria-label="Toggle menu">
            {open ? <X size={18} /> : <Menu size={18} />}
          </button>
        </div>

        {open && (
          <div className="md:hidden pb-6 animate-in">
            <div className="flex flex-col gap-3">
              <a href="#features" className="text-slate-700">Features</a>
              <a href="#testimonials" className="text-slate-700">Customers</a>
              <a href="#pricing" className="text-slate-700">Pricing</a>
              <a href="#faq" className="text-slate-700">FAQ</a>
            </div>
            <div className="mt-4 flex gap-3">
              <a href="https://app.zagosacrm.com/login" className="flex-1 text-center rounded-md border border-slate-300 px-4 py-2 text-slate-700">Sign in</a>
              <a href="https://app.zagosacrm.com/register" className="flex-1 text-center rounded-md bg-slate-900 px-4 py-2 text-white shadow">Get started</a>
            </div>
          </div>
        )}
      </div>
    </header>
  );
}
